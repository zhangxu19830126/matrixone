...
func (a *driverAppender) append(retryTimout, appendTimeout time.Duration) {
	...
	logutil.Debugf("Log Service Driver: append start prepare %p", a.client.record.Payload)
	...
	logutil.Debugf("Log Service Driver: append start %p", a.client.record.Payload)
	...
	logutil.Debugf("Log Service Driver: append end %p", a.client.record.Payload)
	if err != nil {
