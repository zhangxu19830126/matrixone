...
func NewServer(
	...
	s := &server{
		...
		stopper:  stopper.NewStopper("lock-service-rpc-server", stopper.WithLogger(getLogger().RawLogger())),
	}
	s.cfg.Adjust()
...
func (s *server) onMessage(
	...
	v2.TxnRemoteLockQueueSizeGauge.Set(float64(len(s.requests)))
	return nil
}
