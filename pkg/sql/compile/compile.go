...
func (c *Compile) compileApQuery(qry *plan.Query, ss []*Scope, step int32) (*Scope, error) {
	...
	case plan.Query_UPDATE:
		return ss[0], nil
	default:
		rs = c.newMergeScope(ss, []string{"output"})
...
func (c *Compile) compilePlanScope(ctx context.Context, step int32, curNodeIdx int32, ns []*plan.Node) ([]*Scope, error) {
	...
	case plan.Node_DELETE:
		...
		rs := c.newMergeScope(ss, []string{"delete"})
		// updateScopesLastFlag([]*Scope{rs})
		rs.Magic = Merge
		c.setAnalyzeCurrent([]*Scope{rs}, c.anal.curr)
	...
	case plan.Node_ON_DUPLICATE_KEY:
		...
		rs := c.newMergeScope(ss, []string{"ondup"})
	...
	case plan.Node_INSERT:
		...
		if toWriteS3 {
			...
			if !haveSinkScanInPlan(ns, n.Children[0]) && len(ss) != 1 {
				...
				dataScope := c.newMergeScope(ss, []string{"insert"})
				...
				insertArg.ToWriteS3 = true
				rs := c.newMergeScope(scopes, []string{"insert"})
	...
	case plan.Node_LOCK_OP:
		...
		if c.proc.TxnOperator.Txn().IsPessimistic() {
			...
			if block {
				ss = []*Scope{c.newMergeScope(ss, []string{"lockop"})}
			}
		}
		currentFirstFlag := c.anal.isFirst
		for i := range ss {
	...
	case plan.Node_SINK:
		...
		rs := c.newMergeScope(ss, []string{"sink"})
...
func (c *Compile) compileUnion(n *plan.Node, ss []*Scope, children []*Scope) []*Scope {
	...
	mergeChildren := c.newMergeScope(ss, []string{"union"})
...
func (c *Compile) compileUnionAll(ss []*Scope, children []*Scope) []*Scope {
	rs := c.newMergeScope(append(ss, children...), []string{"union all"})
...
func (c *Compile) compileTop(n *plan.Node, topN int64, ss []*Scope) []*Scope {
	...
	for i := range ss {
		...
		if containBrokenNode(ss[i]) {
			ss[i] = c.newMergeScope([]*Scope{ss[i]}, []string{})
		}
		ss[i].appendInstruction(vm.Instruction{
			Op:      vm.Top,
			Idx:     c.anal.curr,
	...
	rs := c.newMergeScope(ss, []string{"top"})
	rs.Instructions[0] = vm.Instruction{
		Op:  vm.MergeTop,
		Idx: c.anal.curr,
...
func (c *Compile) compileOrder(n *plan.Node, ss []*Scope) []*Scope {
	...
	rs := c.newMergeScope(ss, []string{"order"})
	rs.Instructions[0] = vm.Instruction{
		Op:  vm.MergeOrder,
		Idx: c.anal.curr,
...
func (c *Compile) compileWin(n *plan.Node, ss []*Scope) []*Scope {
	rs := c.newMergeScope(ss, []string{"win"})
	rs.Instructions[0] = vm.Instruction{
		Op:  vm.Window,
		Idx: c.anal.curr,
...
func (c *Compile) compileOffset(n *plan.Node, ss []*Scope) []*Scope {
	...
	rs := c.newMergeScope(ss, []string{"offset"})
	rs.Instructions[0] = vm.Instruction{
		Op:  vm.MergeOffset,
		Idx: c.anal.curr,
...
func (c *Compile) compileLimit(n *plan.Node, ss []*Scope) []*Scope {
	...
	rs := c.newMergeScope(ss, []string{"limit"})
	rs.Instructions[0] = vm.Instruction{
		Op:  vm.MergeLimit,
		Idx: c.anal.curr,
...
func (c *Compile) compileMergeGroup(n *plan.Node, ss []*Scope, ns []*plan.Node) []*Scope {
	...
	rs := c.newMergeScope(ss, []string{"group"})
	rs.Instructions[0] = vm.Instruction{
		Op:  vm.MergeGroup,
		Idx: c.anal.curr,
...
func (c *Compile) compileShuffleGroup(n *plan.Node, ss []*Scope, ns []*plan.Node) []*Scope {
	...
	case plan.ShuffleMethod_Reshuffle:
		...
		mergeScopes := c.newMergeScope(ss, []string{"shuffle group"})
...
func (c *Compile) newDeleteMergeScope(arg *deletion.Argument, ss []*Scope) *Scope {
	...
	return c.newMergeScope(rs, []string{"delete merge"})
}

func (c *Compile) newMergeScope(ss []*Scope, producers []string) *Scope {
	rs := &Scope{
		PreScopes: ss,
		Magic:     Merge,
...
func (c *Compile) newMergeRemoteScope(ss []*Scope, nodeinfo engine.Node) *Scope {
	rs := c.newMergeScope(ss, []string{"shuffle join"})
...
func (c *Compile) newJoinScopeListWithBucket(rs, ss, children []*Scope, n *plan.Node) []*Scope {
	currentFirstFlag := c.anal.isFirst
	// construct left
	leftMerge := c.newMergeScope(ss, []string{"bucket join's left"})
	...
	// construct right
	c.anal.isFirst = currentFirstFlag
	rightMerge := c.newMergeScope(children, []string{"bucket join's right"})
...
func (c *Compile) newBroadcastJoinScopeList(ss []*Scope, children []*Scope, n *plan.Node) []*Scope {
	...
	c.anal.isFirst = false
	mergeChildren := c.newMergeScope(children, []string{"broadcastjoin"})
...
func (c *Compile) newInsertMergeScope(arg *insert.Argument, ss []*Scope) *Scope {
	...
	return c.newMergeScope(ss2, []string{"insert merge"})
}
