...
var (
	...
	LogTailNotifyDurationHistogram = prometheus.NewHistogram(
		prometheus.HistogramOpts{
			Namespace: "mo",
			Subsystem: "logtail",
			Name:      "notify_duration_seconds",
			Help:      "Bucketed histogram of notify log tail duration.",
			Buckets:   prometheus.ExponentialBuckets(0.0005, 2.0, 20),
		})

	LogTailNotifyLatencyDurationHistogram = prometheus.NewHistogram(
		prometheus.HistogramOpts{
			Namespace: "mo",
			Subsystem: "logtail",
			Name:      "notify_latency_duration_seconds",
			Help:      "Bucketed histogram of notify log tail latency duration.",
			Buckets:   prometheus.ExponentialBuckets(0.0005, 2.0, 20),
		})

	LogTailCopyPartitionDurationHistogram = prometheus.NewHistogram(
		prometheus.HistogramOpts{
			Namespace: "mo",
			Subsystem: "logtail",
			Name:      "copy_partition_duration_seconds",
			Help:      "Bucketed histogram of notify log tail latency duration.",
			Buckets:   prometheus.ExponentialBuckets(0.0005, 2.0, 20),
		})
